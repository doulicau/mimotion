...
corpid = sys.argv[5]  # 企业ID， 登陆企业微信，在我的企业-->企业信息里查看
corpsecret = sys.argv[6]  # 自建应用，每个自建应用里都有单独的secret
agentid = sys.argv[7]  # 填写你的应用ID，不加引号，是个整型常数,就是AgentId
touser = sys.argv[8]  # 指定接收消息的成员，成员ID列表（多个接收者用‘|’分隔，最多支持1000个）。特殊情况：指定为”@all”，则向该企业应用的全部成员发送
toparty = sys.argv[9]  # 指定接收消息的部门，部门ID列表，多个接收者用‘|’分隔，最多支持100个。当touser为”@all”时忽略本参数
totag = sys.argv[10]  # 指定接收消息的标签，标签ID列表，多个接收者用‘|’分隔，最多支持100个。当touser为”@all”时忽略本参数
...
min_dict = {time_list[0]: 1000, time_list[1]: 2000, time_list[2]: 3000, time_list[3]: 5000, time_list[4]: 7000, time_list[5]: 9000, time_list[6]: 12000}
# 最大步数（例如现在设置意思是在8点（你设置的第一个时间点默认8）运行会在1500到2999中随机生成一个数提交（开启气候降低步数会乘系数K）10点3000~4999。。。以此类推，步数范围建议看懂了再改，没看懂直接默认就好）
max_dict = {time_list[0]: 1999, time_list[1]: 2999, time_list[2]: 3999, time_list[3]: 5999, time_list[4]: 7999, time_list[5]: 9999, time_list[6]: 12999}
# 设置结束
